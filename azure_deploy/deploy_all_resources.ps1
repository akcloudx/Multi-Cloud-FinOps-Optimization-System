...
# seen live NOT rendering visibly in Azure Cloud Shell's browser-based
# terminal even though the underlying process was still correctly waiting on
# stdin for a response - from the outside this looked exactly like a silent
# hang with zero output, and only typing "y" blind (no visible prompt to
# confirm against) unblocked it. Setting this once, up front, tells az to
# always auto-install any required extension silently and continue - the
# standard, documented fix for unattended/scripted az CLI usage (Microsoft's
# own recommendation for CI/automation contexts) - so this class of prompt
# can never appear again, on any terminal, for this or any future az command
# in this script that happens to need a dynamically-installed extension.
