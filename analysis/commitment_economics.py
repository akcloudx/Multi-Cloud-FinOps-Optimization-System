...
def _lookup_rate(prices_df: pd.DataFrame, instrument: str, term: str, region: str, sku: str, os_: str):
...
def _lookup_payg(prices_df: pd.DataFrame, region: str, sku: str, os_: str):
...
def savings_plan_term_comparison(pool_df: pd.DataFrame, prices_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for term in TERMS:
        ...
        for _, r in pool_df.iterrows():
            cached_payg = _lookup_payg(prices_df, r["Region"], r["SKU"], r["OS"])
            payg = cached_payg if cached_payg is not None else float(r["PAYG Hourly Cost USD"])
            payg_total += payg
            rate = _lookup_rate(prices_df, "SavingsPlan", term, r["Region"], r["SKU"], r["OS"])
            if rate is not None:
...
def ri_gap_pricing(coverage_table: pd.DataFrame, inv_raw: pd.DataFrame, prices_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for term in TERMS:
        ...
        for _, row in out.iterrows():
            ...
            cached_payg = _lookup_payg(prices_df, row.get("Region"), row.get("SKU"), row.get("OS"))
            payg = cached_payg if cached_payg is not None else payg_lookup.get(key)
            rate = _lookup_rate(prices_df, "ReservedInstance", term, row.get("Region"), row.get("SKU"), row.get("OS"))
            rates.append(rate)
