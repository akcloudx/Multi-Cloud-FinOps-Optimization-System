...
# Azure SQL Database Serverless - deliberately NOT priced via this lookup,
# disclosed not silent (tried, then reverted live, 2026-09-02). A first
# attempt matched Serverless General Purpose compute to Azure's real
# Retail Prices API (confirmed live: its real armSkuName is a bare
# "{N} vCore", not this app's "GP_S_Gen5_N" SKU string) and found a real,
# non-zero STANDARD retail rate (~$0.52/hr for 1 vCore/westus3). That
# rate is real, but applying it as a flat hourly PAYG cost is WRONG for
# this resource type: Serverless SQL bills per-vCore-SECOND against
# actual usage, auto-pauses when idle, and (this app's own deployed
# database is a live example) can be enrolled in Azure's free-limits
# program - 100,000 free vCore-seconds/month, genuinely $0 up to that
# point. A "$/hr assuming continuous execution" number systematically
# overstates cost for exactly the workloads Serverless is chosen for
# (light/dev/auto-paused) - caught live when it turned a $0.00 database
# into a wrongly-alarming "$380.88/mo" across Inventory/Savings Plan.
# Reverted to leaving it unpriced (returns None below, same as every
# other unpriceable SKU) - matches the SAME deliberate, disclosed choice
# already made for AWS Aurora Serverless v2 (db/aws_seed.py: "PAYG rate
# not currently computed either... a disclosed follow-up rather than a
# silent wrong number"). Precisely modeling Serverless billing (a real
# monthly vCore-second meter, not a flat hourly rate) would need a
# genuinely different data model, not a fix to this function.
